...
if (isset($_SESSION['privilege'])) {
    ...
    <head>
        ...
        <head><title>Demandes valid&eacute;es</title>
            <!--     Fonts and icons     -->
    ...
    <?php include 'navbar.php'; ?>
        ...
        <div class="container"
             style="background:papayawhip;border-radius: 20px; background-color: rgba(149,208,219,0.15)  ;margin-top: 12%;width: 98%;">
            <center>
               ...
               $debut=($page_actuelle-1)*$nb_par_page;
                 $requete = $bdd->prepare('SELECT * FROM events WHERE (valide=1 OR valide=3) AND club_id=? ORDER BY id DESC');
                    $requete->execute(array($_SESSION['id']));

                $nb_results= $requete->rowCount();

                 $requete = $bdd->prepare('SELECT * FROM events WHERE (valide=1 OR valide=3) AND club_id=? ORDER BY id DESC LIMIT '.$debut.','.$nb_par_page);
                    $requete->execute(array($_SESSION['id']));
                ...
                if ($nb_results%$nb_par_page >0) $nb_pages++;
                    ...
                    {
                        echo '<td><center><span class="badge badge-success">Valid&eacute; et s\'est déroul&eacute;</span></center></td>';
                        if ($_SESSION['privilege'] == 1) {?>
                            <td>
                    <center>
                        <button type="button" class="btn btn-info" data-toggle="modal"
                                data-target="#exampleModal<?php echo $resultat['id'] ;?>1">
                            Supprimer de l'historique
                        </button>
                    </center>
                </td><td></td>

                <?php echo '<div class="modal fade" id="exampleModal' . $resultat['id'] . '1" tabindex="-1" role="dialog"
                     aria-labelledby="exampleModalLabel" aria-hidden="true">
                                   <div class="modal-dialog" role="document">
                                       <div class="modal-content">
                                           ...
                                           <div class="modal-body">
                                               <p id="mssg">Voulez-vous vraiment supprimer cet évènement de
                                                   l\'historique?</p>
                                           </div>
                            ...
                            <a class="btn btn-info" href="supprimer_historique.php?id=' . $resultat['id'] . '&reservationsadmin=1">OK</a>
                                           </div>
                                       </div>
                                   </div>
                 </div>';
                    ...
                    {
                    echo '<td><span class="badge badge-success"><center>Valid&eacute; et en attente de d&eacute;roulement</span></center></td>';?>
                    <td>
            <center>
               <button type="button" class="btn btn-primary" data-toggle="modal"
                        data-target="#exampleModal<?php echo $resultat['id'] ?>2">
                    Modifier les salles
                </button>
            </center>
        </td>
        ...
        <!-- Modal -->
        <?php echo '<div class="modal fade" id="exampleModal' . $resultat['id'] . '2" tabindex="-1" role="dialog"
             aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                            ...
                                            <div class="col-6" style="max-width: 30%; right: 32.5%;">
                                                <div class="card">
                                                    <div class="card-body"
                                                        ...
                                                        if ($valide==1)
                                                        {
                                                            $req3=$bdd->prepare('UPDATE events SET valide=1 WHERE id=?'); // on remet le champs validé à 0 (temporairement, juste pour l'affichage des salles)
                                                            $req3->execute(array($res_evt['id']));
                                                        }
                                                        ?>
                                                    </div>
             ...
             <?php echo '<center>
                <button type="button" class="btn btn-warning" data-toggle="modal"
                        data-target="#exampleModal' . $resultat['id'] . '3">
                    Annuler l\'évènement
                </button>
            </center>';?>
        </td>
        ...
        <!-- Modal -->
        <?php echo '<div class="modal fade" id="exampleModal' . $resultat['id'] . '3" tabindex="-1" role="dialog"
             aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                            ...
                            <div class="modal-footer">
                                ...
                                <a class=" btn btn-warning" href="annuler_demande.php?id=' . $resultat['id'] . '">OK</a>
                            </div>
                        </div>
                    </div>
        </div>'; ?>
        ...
        else // donc l'evenemnt est validé puis annulé par l'admin ou par le club : les cas suivants sont traîtés : 1- Le club annule son propre event validé. 2- L'admin annule l'event validé du club. 3-L'admin annule son propre event qui est validé... Dans les trois cas de figures, on affiche à l'utilisateur connecté ses events annulés dans la même page demandes_acceptees. Car même l'admin peut voir ses réservations via demandes_acceptees et donc il pourra voir même celles annulées , on affiche à côté le libellé 'annulé''
        {
            echo '<td><center><span class="badge badge-danger">Valid&eacute; puis annul&eacute;</span></center></td>';
            if ($_SESSION['privilege'] == 1) {?>
                <td>
                    <center>
                    <button type="button" class="btn btn-info" data-toggle="modal"
                                data-target="#exampleModal<?php echo $resultat['id'] ;?>">
                            Supprimer de l'historique
                        </button>
                    </center>
                </td><td></td>

                 <?php echo '<div class="modal fade" id="exampleModal' . $resultat['id'] . '" tabindex="-1" role="dialog"
                     aria-labelledby="exampleModalLabel" aria-hidden="true">
                                   <div class="modal-dialog" role="document">
                                       <div class="modal-content">
                                           <div class="modal-header">
              ...
              <ul class="pagination pg-blue">
                ...
                if ($page_actuelle==1)
                {
                    ...
                    while (($cpt<=3) && ($cpt<=$nb_pages))
                    {
                        ...
                        {
                             echo '<li class="page-item active"><a href="demandes_acceptees.php?page='.$cpt.'" class="page-link">'.$cpt.'</a></li>';
                        }
                        else
                        {
                            echo '<li class="page-item"><a href="demandes_acceptees.php?page='.$cpt.'" class="page-link">'.$cpt.'</a></li>';
                        }
                        $cpt++;
                    ...
                    if ($nb_pages>3)
                    {
                        ?>
                            ...
                            <?php 
                                ...
                                ?>
                          <a href="demandes_acceptees.php?page=<?php echo $nb_pages; ?>" class="page-link" aria-label="Next">
                        ...
                        else
                        {
                            ...
                            ?>
                                ...
                                <?php 
                                    ...
                                    ?>
                              <a class="page-link" href="demandes_acceptees.php" aria-label="Previous">
